...
//#region src/components/date-picker/css-class-plugin.ts
/**
* @param config Plugin configuration.
* @returns A Flatpickr plugin to set CSS classes specific to this design system.
*/
var css_class_plugin_default = (config) => (fp) => {
	/**
	...
	const ensureClassesInDatePicker = () => {
		...
		if (calendarContainer) {
			...
			forEach(calendarContainer.querySelectorAll(selectorFlatpickrDay), (item) => {
				...
				if (item.classList.contains(classFlatpickrToday) && selectedDates.length > 0) item.classList.add(classNoBorder);
				else if (item.classList.contains(classFlatpickrToday) && selectedDates.length === 0) item.classList.remove(classNoBorder);
			});
		}
	};
	/**
...
//#endregion
export { css_class_plugin_default as default };
