...
let CDSDatePickerInput = class CDSDatePickerInput extends FocusMixin(LitElement) {
	...
	*/
	_renderIcon() {
		return this.kind === DATE_PICKER_INPUT_KIND.SIMPLE ? void 0 : iconLoader(Calendar16, {
	...
	*/
	_handleSlotChange({ target }) {
		if (!target.name) this._hasHelperText = target.assignedNodes().some((node) => node.nodeType !== Node.TEXT_NODE || node.textContent.trim());
	}
	render() {
		...
		const invalidIcon = iconLoader(WarningFilled16, { class: `${prefix}--date-picker__icon ${prefix}--date-picker__icon--invalid` });
		const warnIcon = iconLoader(WarningAltFilled16, { class: `${prefix}--date-picker__icon ${prefix}--date-picker__icon--warn` });
...
__decorate([query(`.${prefix}--date-picker__icon`)], CDSDatePickerInput.prototype, "_iconNode", void 0);
...
CDSDatePickerInput = __decorate([carbonElement(`${prefix}-date-picker-input`)], CDSDatePickerInput);
