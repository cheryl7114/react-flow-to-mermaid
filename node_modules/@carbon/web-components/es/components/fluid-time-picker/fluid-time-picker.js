...
import time_picker_default from "../time-picker/time-picker.js";
import fluid_time_picker_default$1 from "./fluid-time-picker.scss.js";
...
*/
let CDSFluidTimePicker = class CDSFluidTimePicker extends time_picker_default {
	constructor(..._args) {
		super(..._args);
		this._hoverTargets = [];
	...
	_syncSelectState() {
		...
		timePickerSelects.forEach((elem, index) => {
			...
			elem.size = this.size === "sm" ? INPUT_SIZE.SMALL : this.size === "lg" ? INPUT_SIZE.LARGE : INPUT_SIZE.MEDIUM;
		});
	}
	...
	render() {
		...
		const fieldOuterWrapperClasses = classMap({ [`${prefix}--text-input__field-outer-wrapper`]: true });
		const fieldWrapperClasses = classMap({ [`${prefix}--text-input__field-wrapper`]: true });
		...
		const validationMessage = normalizedProps.invalid || normalizedProps.warn ? html`
            ...
            ${iconLoader(normalizedProps.invalid ? WarningFilled16 : WarningAltFilled16, { class: `${prefix}--time-picker__icon` })}
          ` : null;
		return html`
	...
	static {
		this.styles = [time_picker_default.styles, fluid_time_picker_default$1];
	}
};
CDSFluidTimePicker = __decorate([carbonElement(`${prefix}-fluid-time-picker`)], CDSFluidTimePicker);
var fluid_time_picker_default = CDSFluidTimePicker;
