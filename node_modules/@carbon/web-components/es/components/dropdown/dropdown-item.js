...
let CDSDropdownItem = class CDSDropdownItem extends LitElement {
	...
	*/
	_getNextItem() {
		...
		while (next) {
			if (next instanceof HTMLElement && (next.tagName.toLowerCase() === `${prefix}-dropdown-item` || next.tagName.toLowerCase() === `${prefix}-combo-box-item`)) return next;
	...
	*/
	...
	connectedCallback() {
		...
		if (!this.hasAttribute("id")) this.setAttribute("id", `${prefix}-dropdown-item-${this.constructor.id++}`);
	...
	*/
	_handleSlotChange({ target }) {
		...
		const textContainer = this.shadowRoot?.querySelector(`.${prefix}--list-box__menu-item__option`);
...
CDSDropdownItem = __decorate([carbonElement(`${prefix}-dropdown-item`)], CDSDropdownItem);
var dropdown_item_default = CDSDropdownItem;
