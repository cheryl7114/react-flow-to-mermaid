...
import slider_default$1 from "./slider.scss.js";
...
let CDSSlider = class CDSSlider extends HostListenerMixin(FormMixin(FocusMixin(LitElement))) {
	...
	*/
	_startDrag(event) {
		if (!this.readonly && !this.disabled) {
			...
			if (!eventContainer) eventContainer = event.target.closest(`.${prefix}--slider__thumb`)?.id ?? "";
	...
	*/
	...
	updated() {
		const sliderfilledTrack = this?.shadowRoot?.querySelector(`.${prefix}--slider__filled-track`);
		if (sliderfilledTrack) if (this.unstable_valueUpper || this.unstable_valueUpper === "") sliderfilledTrack.style.transform = this.unstable_valueUpper ? `translate(${this._rate * 100}%, -50%) scaleX(${this._rateUpper - this._rate})` : `translate(0%, -50%) scaleX(${this._rate})`;
	...
	render() {
      ...
      <div class="${prefix}--slider-container">
        ...
        <div
          ...
          ${unstable_valueUpper || unstable_valueUpper === "" ? html` <div class="${prefix}--slider__filled-track"></div> ` : html` <div class="${prefix}-ce--slider__filled-track-container">
...
CDSSlider = __decorate([carbonElement(`${prefix}-slider`)], CDSSlider);
var slider_default = CDSSlider;
