...
import stack_default$1 from "./stack.scss.js";
...
let CDSStack = class CDSStack extends LitElement {
	...
	updated(changedProperties) {
		if (changedProperties.has("gap")) if (this.useCustomGapValue) this.style.setProperty(`--${prefix}-stack-gap`, `${this.gap}`);
		else {
			const oldGapValue = changedProperties.get("gap");
			this.shadowRoot?.querySelector("div")?.classList.remove(`${prefix}--stack-scale-${oldGapValue}`);
			this.shadowRoot?.querySelector("div")?.classList.add(`${prefix}--stack-scale-${this.gap}`);
		}
	}
	render() {
...
CDSStack = __decorate([carbonElement(`${prefix}-stack`)], CDSStack);
var stack_default = CDSStack;

//#endregion
export { SPACING_STEPS, STACK_ORIENTATION, stack_default as default };
