...
import menu_default$1 from "./menu.scss.js";
import { MenuContext, menuDefaultState } from "./menu-context.js";
import { MENU_BACKGROUND_TOKEN, MENU_SIZE } from "./defs.js";
...
let CDSMenu = class CDSMenu extends HostListenerMixin(LitElement) {
	constructor(..._args) {
		...
		this._fitValue = (range, axis) => {
			const { isRoot } = this.context;
			const { width, height } = (this.shadowRoot?.querySelector(`.${prefix}--menu`) ?? this).getBoundingClientRect();
		...
		this._registerMenuItems = () => {
			...
			else items = this.shadowRoot?.querySelector("slot")?.assignedElements() ?? [];
			...
			this.items?.map((item) => {
				...
				switch (item.tagName) {
					...
					case "CDS-MENU-ITEM-SELECTABLE":
						activeItem = {
							item: item.shadowRoot?.querySelector(`${prefix}-menu-item`),
							parent: item
						};
						this.activeitems = [...this.activeitems, activeItem];
						break;
	...
	*/
	...
	connectedCallback() {
		...
		this.addEventListener("icon-detect", () => {
			this.shadowRoot?.querySelector(".cds--menu")?.classList.add(`${prefix}--menu--with-icons`);
		});
...
CDSMenu = __decorate([carbonElement(`${prefix}-menu`)], CDSMenu);
var menu_default = CDSMenu;

//#endregion
export { MENU_SIZE, menu_default as default };
