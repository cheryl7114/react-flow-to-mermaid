...
import radio_button_default$1 from "./radio-button.scss.js";
...
let CDSRadioButton = class CDSRadioButton extends HostListenerMixin(FocusMixin(LitElement)) {
		...
		this._handleKeydown = (event) => {
			if (!event.target.matches(this.constructor?.aiLabelItem) && !event.target.matches(this.constructor?.slugItem)) {
				...
				if (radioButtonDelegate && manager && !disabled && !disabledItem) {
					const navigationDirection = (orientation === RADIO_BUTTON_ORIENTATION.HORIZONTAL ? navigationDirectionForKeyHorizontal : navigationDirectionForKeyVertical)[event.key];
	...
	*/
	...
	static {
		this.styles = radio_button_default$1;
	}
};
...
CDSRadioButton = __decorate([carbonElement(`${prefix}-radio-button`)], CDSRadioButton);
var radio_button_default = CDSRadioButton;

//#endregion
export { RADIO_BUTTON_LABEL_POSITION, radio_button_default as default };
