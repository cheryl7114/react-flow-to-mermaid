...
import code_snippet_default$1 from "./code-snippet.scss.js";
import "../copy/copy.js";
...
let CDSCodeSnippet = class CDSCodeSnippet extends FocusMixin(LitElement) {
	constructor(..._args) {
		...
		this._resizeObserver = new ResizeObserver(() => {
			const codeContainerRef = this.shadowRoot?.querySelector(`.${prefix}--snippet-container`);
			...
			if (codeContentRef && type === CODE_SNIPPET_TYPE.MULTI || codeContainerRef && type === CODE_SNIPPET_TYPE.SINGLE) handleScroll();
			this.requestUpdate();
		});
		this.copyText = "";
		this.disabled = false;
	...
	*/
	_handleScroll() {
		if (this) {
			...
			if (this.type === CODE_SNIPPET_TYPE.INLINE || this.type === CODE_SNIPPET_TYPE.SINGLE && !codeContainerRef || this.type === CODE_SNIPPET_TYPE.MULTI && !codeContentRef) return;
	...
	*/
	...
	render() {
		...
		if (type) classes += ` ${prefix}--snippet--${type}`;
		if (type !== "inline" && disabled) classes += ` ${prefix}--snippet--disabled`;
		if (hideCopyButton) classes += ` ${prefix}--snippet--no-copy`;
		if (wrapText) classes += ` ${prefix}--snippet--wraptext`;
		if (type == "multi" && hasRightOverflow) classes += ` ${prefix}--snippet--has-right-overflow`;
		...
		if (type === CODE_SNIPPET_TYPE.INLINE) return html`
      ...
      `;
		...
		return html`
      <div
        role="${type === CODE_SNIPPET_TYPE.SINGLE || type === CODE_SNIPPET_TYPE.MULTI ? "textbox" : null}"
        tabindex="${(type === CODE_SNIPPET_TYPE.SINGLE || type === CODE_SNIPPET_TYPE.MULTI) && !disabled ? 0 : null}"
        class="${prefix}--snippet-container"
        aria-label="${"code-snippet"}"
        aria-readonly="${type === CODE_SNIPPET_TYPE.SINGLE || type === CODE_SNIPPET_TYPE.MULTI ? true : null}"
        aria-multiline="${type === CODE_SNIPPET_TYPE.MULTI ? true : null}"
        @scroll="${type === CODE_SNIPPET_TYPE.SINGLE && handleScroll || null}"
        style=${styleMap(styles)}>
        <pre
          @scroll="${type === CODE_SNIPPET_TYPE.MULTI && handleScroll || null}"><code><slot></slot></code></pre>
      </div>
...
CDSCodeSnippet = __decorate([carbonElement(`${prefix}-code-snippet`)], CDSCodeSnippet);
var code_snippet_default = CDSCodeSnippet;

//#endregion
export { FORM_ELEMENT_COLOR_SCHEME as CODE_SNIPPET_COLOR_SCHEME, CODE_SNIPPET_TYPE, code_snippet_default as default };
