...
import link_default$1 from "./link.scss.js";
...
let CDSLink = class CDSLink extends FocusMixin(LitElement) {
	...
	*/
	_handleSlotChange({ target }) {
		const { name } = target;
		this[name === "icon" ? "_hasIcon" : ""] = target.assignedNodes().some((node) => node.nodeType !== Node.TEXT_NODE || node.textContent.trim());
		this.requestUpdate();
	}
	/**
...
CDSLink = __decorate([carbonElement(`${prefix}-link`)], CDSLink);
var link_default = CDSLink;

//#endregion
export { LINK_SIZE, link_default as default };
