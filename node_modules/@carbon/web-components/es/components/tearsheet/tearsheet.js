...
//#region src/components/tearsheet/tearsheet.ts
/**
...
*/
let CDSTearsheet = class CDSTearsheet extends HostListenerMixin(LitElement) {
	static {
		_CDSTearsheet = this;
	}
	constructor(..._args) {
		...
		this._handleBlur = async ({ target, relatedTarget }) => {
			...
			if (open && relatedTarget && oldContains && !currentContains) {
				...
				if (relatedTarget === startSentinelNode || comparisonResult & PRECEDING) {
					...
					if (!tryFocusElems(this.querySelectorAll(selectorTabbableForTearsheet), true) && relatedTarget !== this) this.focus();
				} else if (relatedTarget === endSentinelNode || comparisonResult & FOLLOWING) {
					await this.constructor._delay();
					if (!tryFocusElems(this.querySelectorAll(selectorTabbableForTearsheet), true)) this.focus();
				}
			}
		};
		this._handleKeydown = ({ key, target }) => {
			if ((key === "Esc" || key === "Escape") && this._topOfStack()) this._handleUserInitiatedClose(target);
		};
		this.ariaLabel = "";
		...
		this._checkUpdateActionSizes = () => {
			if (this._actions) for (let i = 0; i < this._actions.length; i++) this._actions[i].setAttribute("size", this.width === "wide" ? "2xl" : "xl");
		};
		this._maxActions = 4;
		this._stackDepth = -1;
		...
		this._notifyStack = () => {
			_CDSTearsheet._stack.all.forEach((handler) => {
				handler(Math.min(_CDSTearsheet._stack.open.length, maxStackDepth), _CDSTearsheet._stack.open.indexOf(handler) + 1);
			});
		};
		...
		this._updateStack = () => {
			if (this.open) _CDSTearsheet._stack.open.push(this._handleStackChange);
			else {
				const indexOpen = _CDSTearsheet._stack.open.indexOf(this._handleStackChange);
				if (indexOpen >= 0) _CDSTearsheet._stack.open.splice(indexOpen, 1);
			}
			this._notifyStack();
		};
		this.actionsMultiple = [
			"",
			"single",
			"double",
			"triple"
		...
		this._checkSetOpen = () => {
			...
			if (tearsheet && this._isOpen) tearsheet.addEventListener("transitionend", () => {
				this._isOpen = false;
			});
	...
	*/
	_handleClickContainer(event) {
		if (event.target.matches(this.constructor.selectorCloseButton)) this._handleUserInitiatedClose(event.target);
	}
	/**
	...
	*/
	...
	_handleSlugChange(e) {
		...
		} else this.removeAttribute("slug");
	}
	...
	connectedCallback() {
		...
		_CDSTearsheet._stack.all.push(this._handleStackChange);
	}
	disconnectedCallback() {
		super.disconnectedCallback();
		const indexAll = _CDSTearsheet._stack.all.indexOf(this._handleStackChange);
		_CDSTearsheet._stack.all.splice(indexAll, 1);
		const indexOpen = _CDSTearsheet._stack.all.indexOf(this._handleStackChange);
		_CDSTearsheet._stack.open.splice(indexOpen, 1);
	}
	render() {
      ...
      width=${width}>
      ${this.width === TEARSHEET_WIDTH.WIDE ? html`<cds-layer level="1" class=${`${blockClass}__header-content`}
      ...
      ${this.hasCloseIcon || this?._actionsCount === 0 ? html`<cds-modal-close-button
      ...
      <div
        ...
        <cds-modal-body class=${`${blockClass}__body`} width=${width}>
          ...
          ${influencerPlacement !== TEARSHEET_INFLUENCER_PLACEMENT.RIGHT ? html`<div
	...
	async updated(changedProperties) {
		if (changedProperties.has("width")) this._checkUpdateActionSizes();
		if (changedProperties.has("width") || changedProperties.has("_hasHeaderNavigation") || changedProperties.has("_hasInfluencerLeft") || changedProperties.has("_hasInfluencerRight") || changedProperties.has("_hasHeaderActions")) {
			if (this.width === "narrow") {
				if (this._hasHeaderNavigation) console.error(`Header navigation is not permitted in narrow Tearsheet.`);
				if (this._hasInfluencerLeft || this._hasInfluencerRight) console.error(`Influencer is not permitted in narrow Tearsheet.`);
				if (this._hasHeaderActions) console.error(`Header actions are not permitted in narrow Tearsheet.`);
			}
		}
...
__decorate([query("#start-sentinel")], CDSTearsheet.prototype, "_startSentinelNode", void 0);
__decorate([query("#end-sentinel")], CDSTearsheet.prototype, "_endSentinelNode", void 0);
__decorate([query(`.${blockClass}__container`)], CDSTearsheet.prototype, "_tearsheet", void 0);
__decorate([queryAssignedElements({
	slot: "actions",
	selector: `${prefix}-button`
})], CDSTearsheet.prototype, "_actions", void 0);
__decorate([state()], CDSTearsheet.prototype, "_actionsCount", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasHeaderActions", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasLabel", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasSlug", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasTitle", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasDescription", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasInfluencerLeft", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasInfluencerRight", void 0);
__decorate([state()], CDSTearsheet.prototype, "_isOpen", void 0);
__decorate([state()], CDSTearsheet.prototype, "_hasHeaderNavigation", void 0);
__decorate([HostListener("click")], CDSTearsheet.prototype, "_handleClick", void 0);
__decorate([HostListener("shadowRoot:focusout")], CDSTearsheet.prototype, "_handleBlur", void 0);
__decorate([HostListener("document:keydown")], CDSTearsheet.prototype, "_handleKeydown", void 0);
__decorate([property({
	reflect: true,
...
__decorate([state()], CDSTearsheet.prototype, "_stackDepth", void 0);
__decorate([state()], CDSTearsheet.prototype, "_stackPosition", void 0);
CDSTearsheet = _CDSTearsheet = __decorate([carbonElement(`${prefix}-tearsheet`)], CDSTearsheet);
var tearsheet_default = CDSTearsheet;

//#endregion
export { TEARSHEET_INFLUENCER_PLACEMENT, TEARSHEET_INFLUENCER_WIDTH, TEARSHEET_WIDTH, tearsheet_default as default };
