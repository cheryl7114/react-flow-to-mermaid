...
let CDSSidePanel = class CDSSidePanel extends HostListenerMixin(LitElement) {
	constructor(..._args) {
		...
		this._checkUpdateIconButtonSizes = () => {
			...
			if (slug || otherButtons?.length) {
				if ((this?.querySelectorAll?.(`${prefix}-button[slot='actions']`))?.length && /l/.test(this.size)) iconButtonSize = "md";
			}
	...
	*/
	...
	_handleSlugChange(e) {
		this._checkUpdateIconButtonSizes();
		this._hasSlug = e.target.assignedElements().length > 0;
	}
	_handleSubtitleChange(e) {
		this._hasSubtitle = (e.target?.assignedElements()).length > 0;
	}
	...
	render() {
      ...
      <div
        ...
        <!-- render back button -->
        ${currentStep > 0 ? html`<cds-icon-button
              align="bottom-left"
...
CDSSidePanel = __decorate([carbonElement(`${prefix}-side-panel`)], CDSSidePanel);
