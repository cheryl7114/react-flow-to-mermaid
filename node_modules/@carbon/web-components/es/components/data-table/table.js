...
import { TABLE_SIZE, TABLE_SORT_DIRECTION } from "./defs.js";
...
let CDSTable = class CDSTable extends HostListenerMixin(LitElement) {
	constructor(..._args) {
		...
		this._handleRowSelect = async (event) => {
			...
			const headerCheckbox = tableHeaderRow.shadowRoot?.querySelector(`${prefix}-checkbox`).shadowRoot.querySelector(`.${prefix}--checkbox`);
		...
		this._handleCancelSelection = async (event) => {
			...
			if (this.contains(target)) tableHeaderRow.shadowRoot?.querySelector(`${prefix}-checkbox`).shadowRoot.querySelector(`.${prefix}--checkbox`).click();
		};
	}
	/**
	* @param lhs A value.
	...
	*/
	...
	_handleSlotChange({ target }) {
		this.withHeader = target.assignedNodes().some((node) => node.nodeType !== Node.TEXT_NODE || node.textContent.trim());
	}
	...
	_handleFilterRows() {
		...
		forEach(this._tableRows, (elem) => {
			...
			if (this.isSelectable) {
				...
				const headerCheckbox = this._tableHeaderRow.shadowRoot?.querySelector(`${prefix}-checkbox`).shadowRoot.querySelector(`.${prefix}--checkbox`);
...
CDSTable = __decorate([carbonElement(`${prefix}-table`)], CDSTable);
var table_default = CDSTable;

//#endregion
export { TABLE_SIZE, table_default as default };
