...
import number_input_default$1 from "./number-input.scss.js";
...
*/
let CDSNumberInput = class CDSNumberInput extends text_input_default {
	constructor(..._args) {
		super(..._args);
	...
	*/
	...
	_getInputValidity() {
		...
		if (this._input?.valueAsNumber > Number(this.max) || this._input?.valueAsNumber < Number(this.min)) return false;
		return true;
	}
	/**
	...
	*/
	...
	render() {
		...
		const invalidIcon = iconLoader(WarningFilled16, { class: `${prefix}--number__invalid` });
		const warnIcon = iconLoader(WarningAltFilled16, { class: `${prefix}--number__invalid ${prefix}--number__invalid--warning` });
    ...
    `;
      ...
      <input
        ...
        @focus="${handleFocus}"
        @blur="${handleBlur}"
        min="${if_non_empty_default(this.min)}"
        max="${if_non_empty_default(this.max)}"
        step="${if_non_empty_default(this.step)}"
        role="alert"
        aria-atomic="true" />
    `;
      ...
      <div class="${wrapperClasses}" ?data-invalid=${normalizedProps.invalid}>
        ...
        ${this.isFluid ? html`<hr class="${prefix}--number-input__divider" />` : null}
	...
	*/
	...
	static {
		this.styles = number_input_default$1;
	}
};
...
CDSNumberInput = __decorate([carbonElement(`${prefix}-number-input`)], CDSNumberInput);
var number_input_default = CDSNumberInput;

//#endregion
export { NUMBER_INPUT_VALIDATION_STATUS, number_input_default as default };
