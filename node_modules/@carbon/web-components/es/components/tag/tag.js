...
import tag_default$1 from "./tag.scss.js";
...
let CDSTag = class CDSTag extends HostListenerMixin(FocusMixin(LitElement)) {
	...
	*/
	_handleIconSlotChange({ target }) {
		this.hasCustomIcon = target.assignedNodes().length > 0;
		this.requestUpdate();
	}
	updated() {
		const textContainer = this.shadowRoot?.querySelector(`.${prefix}--tag__label`);
...
CDSTag = __decorate([carbonElement(`${prefix}-tag`)], CDSTag);
