...
import { forEach, indexOf } from "../../globals/internal/collection-helpers.js";
...
import content_switcher_default$1 from "./content-switcher.scss.js";
import { LitElement, html } from "lit";
import { property } from "lit/decorators.js";

//#region src/components/content-switcher/content-switcher.ts
/**
...
let CDSContentSwitcher = class CDSContentSwitcher extends LitElement {
	...
	*/
	_handleHover({ target, type }) {
		...
		if (target.closest(selectorItem)?.hasAttribute("disabled")) return;
	...
	*/
	_getCurrentItem(target) {
		...
		const { selectorItem } = this.constructor;
		return items[indexOf(items, target.closest(selectorItem))] ?? null;
	}
	/**
	...
	*/
	_handleKeydown({ key }) {
		if (key in NAVIGATION_DIRECTION) this._navigate(NAVIGATION_DIRECTION[key]);
	}
	/**
	...
	*/
	_handleUserInitiatedSelectItem(item, interactionType) {
		if (item && !item.disabled && item.value !== this.value || this.selectionMode === "manual" && interactionType === "keyboard" && !item.disabled) {
			const init = {
				bubbles: true,
				composed: true,
				detail: { item }
			...
			if (this.dispatchEvent(beforeSelectEvent)) {
				...
				const text = item.textContent?.trim() ?? void 0;
	...
	*/
	_navigate(direction) {
		const { selectorItemFocused } = this.constructor;
	...
	*/
	_selectionDidChange(itemToSelect, interactionType) {
		...
		Promise.resolve().then(() => {
			itemToSelect.focus();
			const { selectorItem } = this.constructor;
			const index = indexOf(this.querySelectorAll(selectorItem), itemToSelect.closest(selectorItem));
			const nextIndex = index < 0 ? index : index + 1;
			forEach(this.querySelectorAll(selectorItem), (elem, i) => {
				elem.hideDivider = i === nextIndex;
			});
	...
	shouldUpdate(changedProps) {
		if (changedProps.has("iconOnly") || changedProps.has("selectedIndex")) {
			...
			this.iconOnly = Array.from(items).every((item) => item.hasAttribute("icon"));
		}
		return true;
	}
	updated(changedProperties) {
		if (changedProperties.has("selectedIndex")) this._updateSelectedItemFromIndex();
		this._updateSelectedItemFromValue(changedProperties);
		if (changedProperties.has("size")) this.querySelectorAll(`${prefix}-content-switcher-item`).forEach((item) => {
	...
	*/
	...
	static {
		this.styles = content_switcher_default$1;
	}
};
__decorate([property({ reflect: true })], CDSContentSwitcher.prototype, "value", void 0);
__decorate([property({
	type: Number,
	attribute: "selected-index"
})], CDSContentSwitcher.prototype, "selectedIndex", void 0);
__decorate([property({ attribute: "selection-mode" })], CDSContentSwitcher.prototype, "selectionMode", void 0);
__decorate([property({ reflect: true })], CDSContentSwitcher.prototype, "size", void 0);
__decorate([property({
	type: Boolean,
	reflect: true,
	attribute: "icon"
...
CDSContentSwitcher = __decorate([carbonElement(`${prefix}-content-switcher`)], CDSContentSwitcher);
var content_switcher_default = CDSContentSwitcher;

//#endregion
export { CONTENT_SWITCHER_SIZE, NAVIGATION_DIRECTION, content_switcher_default as default };
