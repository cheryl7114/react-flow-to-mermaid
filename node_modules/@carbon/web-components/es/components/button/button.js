...
import button_default$1 from "./button.scss.js";
...
let CDSButton = class CDSButton extends HostListenerMixin(FocusMixin(LitElement)) {
	...
	*/
	_handleSlotChange({ target }) {
		const { name } = target;
		this[name === "icon" ? "_hasIcon" : "hasMainContent"] = target.assignedNodes().some((node) => node.nodeType !== Node.TEXT_NODE || node.textContent.trim());
		this.requestUpdate();
	}
	...
	_checkBadgeWarning() {
		if (this.querySelector(`${prefix}-badge-indicator`) && (this.kind !== BUTTON_KIND.GHOST || this.size !== BUTTON_SIZE.LARGE)) console.warn(`The badge indicator must be used with kind='ghost' and size='lg'`);
	}
	...
	render() {
        ...
        ` : html`
          <button
            ...
            aria-label="${ifDefined(tooltipText)}"
            aria-describedby="badge-indicator">
            ${isDanger ? html`<span class="${prefix}--visually-hidden"
...
CDSButton = __decorate([carbonElement(`${prefix}-button`)], CDSButton);
var button_default = CDSButton;

//#endregion
export { BUTTON_KIND, BUTTON_SIZE, BUTTON_TOOLTIP_ALIGNMENT, BUTTON_TOOLTIP_POSITION, BUTTON_TYPE, button_default as default };
