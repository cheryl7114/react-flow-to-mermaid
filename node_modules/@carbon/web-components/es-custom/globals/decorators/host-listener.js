...
//#region src/globals/decorators/host-listener.ts
/**
...
const setHostListener = (type, options, Clazz, name) => {
	...
	if (!hostListeners) throw new Error("The method `@HostListener()` is defined on has to be of a class that has `HostListerMixin`.");
	if (!hostListeners[name]) hostListeners[name] = {};
...
const HostListenerStandard = (type, options, descriptor) => {
	...
	if (!(kind === "method" && placement === "prototype" || kind === "field" && placement === "own")) throw new Error("`@HostListener()` must be defined on instance methods, but you may have defined it on static, field, etc.");
	return {
		...descriptor,
		finisher(Clazz) {
