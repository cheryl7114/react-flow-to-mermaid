...
//#region src/globals/internal/radio-group-manager.ts
/**
...
*/
let NAVIGATION_DIRECTION = /* @__PURE__ */ function(NAVIGATION_DIRECTION) {
	/**
	* Navigating backward.
	*/
...
*/
var RadioGroupManager = class RadioGroupManager {
	...
	*/
	shouldBeFocusable(radio) {
		...
		if (group && Array.from(group).some((item) => item.checked)) return false;
		return !group || group.size === 1 || this.getSortedGroup(radio)[0] === radio;
	}
	/**
	* @param radio A radio button.
	...
	*/
	getSortedGroup(radio) {
		...
		return group && Array.from(group).sort((lhs, rhs) => {
			...
			if (comparisonResult & Node.DOCUMENT_POSITION_FOLLOWING || comparisonResult & Node.DOCUMENT_POSITION_CONTAINED_BY) return -1;
			if (comparisonResult & Node.DOCUMENT_POSITION_PRECEDING || comparisonResult & Node.DOCUMENT_POSITION_CONTAINS) return 1;
			return 0;
		});
	}
	/**
	...
	*/
	navigate(radio, direction) {
		...
		for (let i = 0; i < n; i++) {
			...
			if (!candidateRadio.disabled) return candidateRadio;
		}
		return radio;
	}
	static {
		this._instances = /* @__PURE__ */ new WeakMap();
	}
	/**
