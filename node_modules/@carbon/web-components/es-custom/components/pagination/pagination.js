...
import pagination_default$1 from "./pagination.scss.js";
...
let CDSPagination = class CDSPagination extends FocusMixin(HostListenerMixin(LitElement)) {
	...
	*/
	_handleClickPrevButton() {
		...
		if (this.page === 1) {
			const { selectorForwardButton } = this.constructor;
			(this.shadowRoot?.querySelector(`[button-class-name*=${selectorForwardButton}]`)).focus();
		}
	}
	/**
	...
	*/
	_handleClickNextButton() {
		...
		if (!pagesUnknown && this.page === this.totalPages) {
			const { selectorPreviousButton } = this.constructor;
			(this.shadowRoot?.querySelector(`[button-class-name*=${selectorPreviousButton}]`)).focus();
		}
	}
	/**
	...
	*/
	...
	updated(changedProperties) {
		...
		const { selectorPageSizesSelect, selectorPagesSelect } = this.constructor;
		if (changedProperties.has("pageSize")) {
			const pageSizeSelect = this.shadowRoot?.querySelector(selectorPageSizesSelect);
		...
		if (changedProperties.has("pageSize") || changedProperties.has("start") || changedProperties.has("totalItems") || changedProperties.has("page")) {
			...
			const pagesSelect = this.shadowRoot?.querySelector(selectorPagesSelect);
			if (pagesSelect) pagesSelect.value = displayPage.toString();
		}
		if (changedProperties.has("page")) {
			this.start = this._calculateStart(this.page, this.pageSize, this.totalItems, this.pagesUnknown);
			const pagesSelect = this.shadowRoot?.querySelector(this.constructor.selectorPagesSelect);
			if (pagesSelect) pagesSelect.value = String(this.page);
		}
	}
	render() {
	...
	*/
	...
	static {
		this.styles = pagination_default$1;
	}
};
__decorate([query(`${prefix}-select`)], CDSPagination.prototype, "_pageSizeSelect", void 0);
__decorate([HostListener(`${prefix}-select-selected`)], CDSPagination.prototype, "_handleChangeSelector", null);
...
CDSPagination = __decorate([carbonElement(`${prefix}-pagination`)], CDSPagination);
var pagination_default = CDSPagination;
