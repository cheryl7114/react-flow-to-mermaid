...
import progress_bar_default$1 from "./progress-bar.scss.js";
...
let CDSProgressBar = class CDSProgressBar extends LitElement {
	...
	get _cappedValue() {
		...
		if (status === PROGRESS_BAR_STATUS.ERROR) cappedValue = 0;
		else if (status === PROGRESS_BAR_STATUS.FINISHED) cappedValue = max;
		return cappedValue;
	}
	updated(changedProperties) {
		if (changedProperties.has("value") || changedProperties.has("max") || changedProperties.has("status")) {
			...
			if (status != PROGRESS_BAR_STATUS.ERROR && status != PROGRESS_BAR_STATUS.FINISHED) bar.style.transform = `scaleX(${percentage})`;
	...
	render() {
		...
		let statusIcon = null;
		if (isError) statusIcon = iconLoader(ErrorFilled16, { class: `${prefix}--progress-bar__status-icon` });
		else if (isFinished) statusIcon = iconLoader(CheckmarkFilled16, { class: `${prefix}--progress-bar__status-icon` });
      ...
      ${helperText ? html`<div class="${prefix}--progress-bar__helper-text">
            ${helperText}
	...
	static {
		this.styles = progress_bar_default$1;
	}
};
__decorate([property({
	type: String,
...
CDSProgressBar = __decorate([carbonElement(`${prefix}-progress-bar`)], CDSProgressBar);
var progress_bar_default = CDSProgressBar;

//#endregion
export { PROGRESS_BAR_SIZE, PROGRESS_BAR_STATUS, PROGRESS_BAR_TYPE, progress_bar_default as default };
