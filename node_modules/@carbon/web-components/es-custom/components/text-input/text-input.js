...
import text_input_default$1 from "./text-input.scss.js";
...
let CDSTextInput = class CDSTextInput extends ValidityMixin(FormMixin(LitElement)) {
	...
	*/
	...
	updated() {
		...
		if (label) label?.classList.toggle(`${prefix}--slug--revert`, this.querySelector(`${prefix}-ai-label`)?.hasAttribute("revert-active"));
		else this.shadowRoot?.querySelector("slot[name='slug']")?.classList.toggle(`${prefix}--slug--revert`, this.querySelector(`${prefix}-slug`)?.hasAttribute("revert-active"));
	}
	/**
	...
	*/
	...
	static {
		this.styles = text_input_default$1;
	}
};
...
CDSTextInput = __decorate([carbonElement(`${prefix}-text-input`)], CDSTextInput);
var text_input_default = CDSTextInput;

//#endregion
export { FORM_ELEMENT_COLOR_SCHEME as INPUT_COLOR_SCHEME, INPUT_SIZE, INPUT_TOOLTIP_ALIGNMENT, INPUT_TOOLTIP_DIRECTION, INPUT_TYPE, text_input_default as default };
