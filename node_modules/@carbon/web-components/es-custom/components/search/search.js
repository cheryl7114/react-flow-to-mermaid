...
import search_default$1 from "./search.scss.js";
...
let CDSSearch = class CDSSearch extends HostListenerMixin(FocusMixin(FormMixin(LitElement))) {
	...
	*/
	_handleExpand(e) {
		if ((e.composedPath && e.composedPath() || []).some((n) => n?.classList?.contains(`${prefix}--search-magnifier`)) && this.expandable && !this.expanded) this._expandAndFocus();
	}
	_expandAndFocus() {
	...
	*/
	_focusMagnifier() {
		const magnifier = this.shadowRoot?.querySelector(`.${prefix}--search-magnifier`);
		if (magnifier) {
			if (!magnifier.hasAttribute("tabindex")) magnifier.tabIndex = -1;
			magnifier.focus();
		}
	}
	render() {
...
CDSSearch = __decorate([carbonElement(`${prefix}-search`)], CDSSearch);
var search_default = CDSSearch;
