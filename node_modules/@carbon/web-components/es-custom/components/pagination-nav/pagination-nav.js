...
import pagination_nav_default$1 from "./pagination-nav.scss.js";
...
//#region src/components/pagination-nav/pagination-nav.ts
/**
*
* Copyright IBM Corp. 2026
*
...
let CDSPaginationNav = class CDSPaginationNav extends LitElement {
	...
	_pageWouldBeHidden(page) {
		const wouldBeHiddenInFront = page >= (this.itemsDisplayedOnPage <= 4 && this.page > 1 ? 0 : 1) && page <= this.cuts.front || page === 0;
	...
	*/
	_decrementIndex() {
		...
		if (loop) this.page = wouldLoop ? totalItems - 1 : page - 1;
		else this.page = Math.max(this.page - 1, 0);
		this.dispatchEvent(new CustomEvent(this.constructor.eventChange, {
			bubbles: true,
			composed: true,
	...
	*/
	_incrementIndex() {
		...
		if (loop) this.page = wouldLoop ? 0 : page + 1;
		else this.page = Math.min(this.page + 1, this.totalItems - 1);
		this.dispatchEvent(new CustomEvent(this.constructor.eventChange, {
			bubbles: true,
			composed: true,
			detail: { page: this.page }
	...
	*/
        ...
        <li class="${prefix}--pagination-nav__list-item">
          <div class="${prefix}--pagination-nav__select">
            ...
            <div class="${prefix}--pagination-nav__select-icon-wrapper">
              ${iconLoader(OverflowMenuHorizontal16, { class: `${prefix}--pagination-nav__select-icon` })}
            </div>
          </div>
        </li>
      `;
		else if (count === 1) return this._renderPaginationItem(fromIndex);
		else return html``;
	}
	render() {
      ...
      <nav class="${classMap({
        ...
        <ul class="${prefix}--pagination-nav__list">
          ...
          ${Array.from({ length: this.totalItems }).map((_, i) => i).slice(startOffset + this.cuts.front, (1 + this.cuts.back) * -1).map((item) => this._renderPaginationItem(item, this.page === item))}
	...
	shouldUpdate(changedProperties) {
		if (changedProperties.has("totalItems") || changedProperties.has("page")) {
			if (this.page > this.totalItems) this.page = this.totalItems - 1;
			if (this.page < 0) this.page = 0;
		}
		return true;
	}
	updated(changedProperties) {
		if (changedProperties.has("page")) this.prevPage = changedProperties.get("page");
		...
		if (changedProperties.has("page") && this._pageWouldBeHidden(this.page)) if (this.page - (this.prevPage || 0) > 0) {
	...
	static {
		this.styles = pagination_nav_default$1;
	}
};
__decorate([state()], CDSPaginationNav.prototype, "itemsDisplayedOnPage", void 0);
__decorate([state()], CDSPaginationNav.prototype, "cuts", void 0);
__decorate([state()], CDSPaginationNav.prototype, "prevPage", void 0);
__decorate([property({
...
__decorate([property({ reflect: true })], CDSPaginationNav.prototype, "size", void 0);
__decorate([property({
...
__decorate([property({ attribute: false })], CDSPaginationNav.prototype, "translateWithId", void 0);
CDSPaginationNav = __decorate([carbonElement(`${prefix}-pagination-nav`)], CDSPaginationNav);
var pagination_nav_default = CDSPaginationNav;
