...
import textarea_default$1 from "./textarea.scss.js";
...
*/
let CDSTextarea = class CDSTextarea extends text_input_default {
	constructor(..._args) {
		super(..._args);
		this.isFluid = false;
	...
	*/
	...
	render() {
		...
		const invalidIcon = iconLoader(WarningFilled16, { class: `${prefix}--text-area__invalid-icon` });
		const warnIcon = iconLoader(WarningAltFilled16, { class: `${prefix}--text-area__invalid-icon ${prefix}--text-area__invalid-icon--warning` });
	...
	updated() {
		...
		const wrapper = this.shadowRoot?.querySelector(`.${prefix}--text-area__wrapper`);
		if (!wrapper) return;
	...
	*/
	_measureWrapper() {
		const wrapperWidth = (this.shadowRoot?.querySelector(`.${prefix}--text-area__wrapper`))?.scrollWidth;
		[this.shadowRoot?.querySelector(`.${prefix}--form__helper-text`), this.shadowRoot?.querySelector(`.${prefix}--form-requirement`)].forEach((el) => {
			if (el) {
	...
	static {
		this.styles = textarea_default$1;
	}
};
...
CDSTextarea = __decorate([carbonElement(`${prefix}-textarea`)], CDSTextarea);
var textarea_default = CDSTextarea;
