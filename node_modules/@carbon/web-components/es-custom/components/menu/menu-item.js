...
import menu_item_default$1 from "./menu-item.scss.js";
...
let CDSmenuItem = class CDSmenuItem extends HostListenerMixin(HostListenerMixin(LitElement)) {
	constructor(..._args) {
				...
				for (const mutation of mutationsList) if (mutation.type === "childList") {
					...
					if (item) switch (item.tagName) {
						...
						case "CDS-MENU-ITEM-GROUP": {
							...
							this.submenuEntry = slotElements?.length && slotElements[0];
							break;
						}
						case "CDS-MENU-ITEM":
		...
		this._closeSubmenu = () => {
			...
			this.submenuOpen = false;
			(this.shadowRoot?.querySelector(`.${prefix}--menu-item`))?.focus();
		};
	...
	updated() {
		...
		if (this.kind === MENU_ITEM_KIND.DANGER) this.classList.toggle(`${prefix}--menu-item--danger`);
	}
	handleClick(event) {
	...
	render() {
		...
		const menuClassName = this.context?.hasSelectableItems ? `${prefix}--menu--with-selectable-items` : "";
		return html`
      ...
      ${isDanger ? html`<span id="danger-description" class="${prefix}--visually-hidden"
            >${dangerDescription}</span
          >` : html``}
      ${shortcut && !this.hasSubmenu ? html` <div class="${prefix}--menu-item__shortcut">${shortcut}</div> ` : html``}
      ${this.hasSubmenu ? html`
            ...
            <cds-custom-menu
              ...
              size=${this.parentElement?.getAttribute("size") ?? MENU_SIZE.LARGE}
	...
	static {
		this.styles = menu_item_default$1;
	}
};
...
CDSmenuItem = __decorate([carbonElement(`${prefix}-menu-item`)], CDSmenuItem);
var menu_item_default = CDSmenuItem;

//#endregion
export { MENU_CLOSE_ROOT_EVENT, MENU_ITEM_KIND, MENU_SIZE, menu_item_default as default };
