...
import { BUTTON_KIND, BUTTON_SIZE, BUTTON_TOOLTIP_POSITION } from "../button/defs.js";
...
*/
let CDSHeaderGlobalAction = class CDSHeaderGlobalAction extends button_default {
	constructor(..._args) {
		super(..._args);
	...
	_handleClick(event) {
		...
		if (disabled) event.stopPropagation();
			...
			const panel = this.ownerDocument?.querySelector(`#${this.panelId}`);
			if (panel) {
				if (panel.getAttribute("expanded")) panel.removeAttribute("expanded");
				else panel.setAttribute("expanded", "true");
...
CDSHeaderGlobalAction = __decorate([carbonElement(`${prefix}-header-global-action`)], CDSHeaderGlobalAction);
var header_global_action_default = CDSHeaderGlobalAction;

//#endregion
export { header_global_action_default as default };
