...
import tree_view_default$1 from "./tree-view.scss.js";
...
*/
let CDSTreeView = class CDSTreeView extends HostListenerMixin(LitElement) {
	static {
		_CDSTreeView = this;
	}
	constructor(..._args) {
		...
		this._click = ({ target }) => {
			...
			this.querySelectorAll(_CDSTreeView.selectorTreeNode).forEach((node) => {
		...
		this._handleKeyDown = (event) => {
			const { key } = event;
			const nodes = Array.from(this.querySelectorAll(_CDSTreeView.selectorTreeNode)).filter((node) => node.checkVisibility() && !node.hasAttribute("disabled"));
			const allNodes = Array.from(this.querySelectorAll(_CDSTreeView.selectorTreeNode)).filter((node) => !node.hasAttribute("disabled"));
	...
	async _setInitialFocus() {
		...
		const nodes = this.querySelectorAll(_CDSTreeView.selectorTreeNode);
		if (nodes.length > 0) {
	...
	updated(changedProperties) {
		...
		if (changedProperties.has("size")) this.querySelectorAll(_CDSTreeView.selectorTreeNode).forEach((item) => {
	...
	render() {
		...
		return html`
      ${!hideLabel ? html`<label id=${labelId} class=${`${prefix}--label`}
              >${label}
            </label>` : null}
      <ul
	...
	static {
		this.styles = tree_view_default$1;
	}
};
__decorate([property({
	type: Boolean,
...
CDSTreeView = _CDSTreeView = __decorate([carbonElement(`${prefix}-tree-view`)], CDSTreeView);
var tree_view_default = CDSTreeView;

//#endregion
export { TREE_SIZE, tree_view_default as default };
