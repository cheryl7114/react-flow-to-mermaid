...
* Date picker modes.
*/
var DATE_PICKER_MODE = /* @__PURE__ */ function(DATE_PICKER_MODE) {
	/**
...
let CDSDatePicker = class CDSDatePicker extends HostListenerMixin(FormMixin(LitElement)) {
	...
	*/
	get _mode() {
		...
		if (this.querySelector(selectorInputTo)) return DATE_PICKER_MODE.RANGE;
		if (this.querySelector(`${prefix}-date-picker-input[kind="single"]`)) return DATE_PICKER_MODE.SINGLE;
	...
	*/
	get _datePickerPlugins() {
		...
		if (mode === DATE_PICKER_MODE.RANGE) plugins.push(range_plugin_default({ input: inputTo }));
		return plugins;
	}
	/**
...
CDSDatePicker = __decorate([carbonElement(`${prefix}-date-picker`)], CDSDatePicker);
