...
import popover_default$1 from "./popover.scss.js";
...
*/
let CDSPopover = class CDSPopover extends HostListenerMixin(LitElement) {
	static {
		_CDSPopover = this;
	}
	/**
	...
	*/
	_handleSlotChange({ target }) {
		if (this.tabTip) target.assignedNodes().filter((node) => node.nodeType === Node.ELEMENT_NODE && node.tagName === "BUTTON")[0]?.classList.add(`${prefix}--popover--tab-tip__button`);
		this.requestUpdate();
	}
	...
	_handleFocusOut(event) {
		...
		if (this.contains(event.relatedTarget) || deepShadowContains(this, event.relatedTarget)) {
	...
	*/
	...
	updated(changedProperties) {
		...
		if (this.autoalign && this.open) {
			...
			const tooltip = content?.shadowRoot?.querySelector(_CDSPopover.selectorPopoverContentClass);
			const arrowElement = content?.shadowRoot?.querySelector(_CDSPopover.selectorPopoverCaret);
...
CDSPopover = _CDSPopover = __decorate([carbonElement(`${prefix}-popover`)], CDSPopover);
var popover_default = CDSPopover;
