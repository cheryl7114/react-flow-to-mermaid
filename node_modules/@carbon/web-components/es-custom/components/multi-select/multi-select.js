...
import dropdown_default from "../dropdown/dropdown.js";
import { SELECTION_FEEDBACK_OPTION } from "./defs.js";
import multi_select_default$1 from "./multi-select.scss.js";
...
let CDSMultiSelect = class CDSMultiSelect extends dropdown_default {
	constructor(..._args) {
		...
		this.sortItems = (menuItems, { values, compareItems, locale = "en" }) => {
			return Array.from(menuItems).sort((itemA, itemB) => {
				...
				return compareItems(itemA.value, itemB.value, { locale });
			});
		};
	}
	...
	_handleClickInner(event) {
		...
		if (this._selectionButtonNode?.contains(event.target) && !this.readOnly) {
			...
			this.querySelectorAll(`${prefix}-multi-select-item`).forEach((el) => el.removeAttribute("highlighted"));
	...
	updated(changedProperties) {
		...
		if (changedProperties.has("open") && this.open) {
			...
			if (selectedItems.length > 0) {
				...
				if (this.selectAll) itemToFocus = this.querySelector(`${prefix}-multi-select-item[is-select-all]`);
	...
	*/
	...
	static {
		this.styles = multi_select_default$1;
	}
};
...
__decorate([query(`.${prefix}--list-box__field`)], CDSMultiSelect.prototype, "_triggerNode", void 0);
...
CDSMultiSelect = __decorate([carbonElement(`${prefix}-multi-select`)], CDSMultiSelect);
var multi_select_default = CDSMultiSelect;

//#endregion
export { DROPDOWN_DIRECTION, DROPDOWN_SIZE, DROPDOWN_TYPE, SELECTION_FEEDBACK_OPTION, multi_select_default as default };
