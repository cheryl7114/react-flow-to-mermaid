...
let CDSSelect = class CDSSelect extends FormMixin(LitElement) {
	...
	*/
	...
	updated(changedProperties) {
		...
		if (label) {
			if (label.assignedNodes()?.length) this._inputNode?.classList.add(`${prefix}--select-input-has--ai-label`);
			label?.classList.toggle(`${prefix}--slug--revert`, this.querySelector(`${prefix}-ai-label`)?.hasAttribute("revert-active"));
	...
	render() {
        ...
        ` : null;
		const errorText = normalizedProps.invalid || normalizedProps.warn ? html` <div id="error-text" class="${prefix}--form-requirement">
      ...
      ${!normalizedProps.invalid ? void 0 : iconLoader(WarningFilled16, { class: `${prefix}--select__invalid-icon` })}
      ${!normalizedProps.invalid && normalizedProps.warn ? iconLoader(WarningAltFilled16, { class: `${prefix}--select__invalid-icon ${prefix}--select__invalid-icon--warning` }) : null}
    `;
		return html`
	...
	*/
	...
	static {
		this.styles = select_default$1;
	}
};
__decorate([query("select")], CDSSelect.prototype, "_selectNode", void 0);
__decorate([query("#input")], CDSSelect.prototype, "_inputNode", void 0);
...
CDSSelect = __decorate([carbonElement(`${prefix}-select`)], CDSSelect);
